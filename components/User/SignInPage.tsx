...
import { loginUser, promptNotDeployed } from "@/utils/localAuth";
...
const SignInPage = ({ providers, csrfToken, callbackUrl }: any) => {
    ...
    const signInHandler = async () => {
        setLoading(true);
        const res = loginUser(login_email, login_password);

        if (!res.success) {
            setLoading(false);
            setUser({
                ...user,
                login_error: res.message || "Invalid credentials",
            });
        } else {
            setTimeout(() => {
                setLoading(false);
        ...
        <>
            ...
            <div className="flex flex-col mx-auto w-full px-4 sm:w-3/5 md:w-3/5 lg:w-2/5 pt-8 pb-16">
                ...
                <div className="flex flex-col p-4 my-4 bg-white rounded border space-y-4">
                    ...
                    <div className="flex">
                        {login_error && (
                            <p className="text-red-500 text-sm font-semibold">{login_error}</p>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row">
                        {providers && Array.isArray(providers) && providers.map((provider: any) => {
                            ...
                            return (
                                <div
                                    onClick={() => handleOtherOption(`Sign in with ${provider.name}`)}
                                    key={provider.name}
                                    className="flex bg-white items-center w-full p-2 rounded-xl border mt-3 md:mt-1 mx-2 cursor-pointer hover:bg-gray-50"
                                >
                                    <Image
                        ...
                        {needHelp && (
                            <div className="flex flex-col ml-4 mt-2 space-y-2">
                                <button
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        handleOtherOption("Forgot password");
                                    }}
                                    className="text-left text-blue-500 hover:text-amazon-orange hover:underline"
                                >
                                ...
                                <button
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        handleOtherOption("Sign-In support");
                                    }}
                                    className="text-left text-blue-500 hover:text-amazon-orange hover:underline"
                                >
