...
import { mockProducts, mockCategories, mockSubCategories } from "@/utils/localData";
...
const Browse = ({
    ...
    const priceHandler = (price: any, type: any, delay: any) => {
        let rawPrice = router.query.price;
        let priceStr = Array.isArray(rawPrice) ? rawPrice[0] : rawPrice || "";
        let priceQuery = priceStr.split("_");
    ...
    const replaceQuery = (queryName: any, value: any) => {
        const raw = router.query[queryName];
        const existedQeury = Array.isArray(raw) ? raw[0] : raw || "";
        const valueCheck = existedQeury ? existedQeury.search(value) : -1;
        const _check = existedQeury ? existedQeury.search(`_${value}`) : -1;
        let result: any = null;
        if (existedQeury) {
        ...
        return {
            result,
            active: Boolean(existedQeury && valueCheck !== -1),
        };
    };
    // ----------------------------------------
    ...
    useEffect(() => {
        ...
        const h2 = el.current?.offsetHeight || 0;
        setHeight(h1 + h2 + 50);
...
export async function getServerSideProps(context: any) {
        ...
        if (products && products.length > 0) {
            return {
                props: {
                    ...
                    styles: styles || [],
                    materials: materials || [],
                    paginationCount: Math.ceil((totalProducts || 0) / pageSize),
                },
            };
        }
    } catch (error) {
        console.error("Using local data fallback for browse page.");
    }

    const mockColors = Array.from(
        new Set(mockProducts.flatMap((p) => p.subProducts.map((sp) => sp.color.color)))
    );
    const mockBrands = Array.from(new Set(mockProducts.map((p) => p.brand)));
    const mockSizes = Array.from(
        new Set(
            mockProducts.flatMap((p) =>
                p.subProducts.flatMap((sp) => sp.sizes.map((s) => s.size))
            )
        )
    );
    ...
    return {
        props: {
            ...
            colors: mockColors,
            brands: mockBrands,
            styles: ["Casual", "Sportswear", "Denim"],
            materials: ["Cotton", "Mesh", "Leather"],
            paginationCount: 1,
        },
    };
}
